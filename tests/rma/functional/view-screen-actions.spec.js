...
const EXPECTED = {
  Submitted: {
    ...
    customer:      { edit: true, comment: true, accept: false, reject: false, receive: false, repair: false, onHold: false, close: false, printConsign: false, emailConsign: false, print: false, back: true },
  },
  Accepted: {
...
function generateViewScreenTests(roleName, storageKey, matrixKey) {
  ...
  test.describe(`View Screen — ${roleName} @view-screen`, () => {
    ...
    for (const [status, roleMap] of Object.entries(EXPECTED)) {
      ...
      test(`VS-${status.replace(/\s/g, '')}-${roleName.replace(/\s/g, '')} | ${roleName}: View Screen in "${status}" status`, async ({ page }) => {
        const { vrPage, found } = await openRmaByStatus(page, status === 'On Hold' ? 'On-Hold' : status);
        if (!found) { test.skip(true, `No "${status}" RMA found`); return; }

        const actual = await vrPage.getDetailAllActionButtons();

        // Log the full button map for debugging
        const summary = Object.entries(actual)
          .filter(([, v]) => v)
          .map(([k]) => BTN_LABELS[k] || k)
          .join(', ');
        console.log(`  ${roleName} on ${status}: [${summary}]`);

        // Assert each button
        for (const [btnKey, shouldBeVisible] of Object.entries(expected)) {
          expect(
            actual[btnKey],
            `${roleName} on "${status}": "${BTN_LABELS[btnKey]}" should be ${shouldBeVisible ? 'VISIBLE' : 'HIDDEN'}`
...
test.describe('View Screen — Email Consign. Note Action @view-screen', () => {
  ...
  test('VS-EMAIL-CN | Email Consign. Note shows success message', async ({ page }) => {
    ...
    for (const status of statusesWithEmailCN) {
      ...
      if (result.found) {
        ...
        if (btns.emailConsign) { found = true; break; }
      }
    }

    if (!found) { test.skip(true, 'No RMA with Email Consign. Note button found'); return; }
    ...
    console.log(`  Email Consign. Note success: "${successMsg}" ✓`);
  });
});
